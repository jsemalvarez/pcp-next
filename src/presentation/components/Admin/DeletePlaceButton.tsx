...
export function DeletePlaceButton({ id, name }: { id: string; name: string }) {
  ...
  const handleDelete = async () => {
    if (window.confirm(`¿Estás seguro de que deseas eliminar el lugar "${name}"? Esta acción no se puede deshacer.`)) {
      setIsDeleting(true);
      const result = await deletePlace(id);
      if (!result.success) {
        alert(result.error || "Hubo un error al eliminar el lugar.");
        setIsDeleting(false);
      }
    }
  };
